...
const Formulario = ({pacientes, setPacientes, paciente, setPaciente}) => {  
  ...
  // when button edit is clicked put paciente info on the fields of the form
  useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    if ([nombre, propietario, email, alta, sintomas].includes("")) {
      ...
      if (paciente.id) {
        ...
        //good practice for add on array
    ...
    <div className="md:w-1/2 lg:w-2/5">
      ...
      <form onSubmit={handleSubmit} className="bg-white shadow-md rounded-lg py-10 px-5 mb-10">
        ...
        <div className="mb-5">
          ...
          <input id="email" type="email" className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md" placeholder="Email" value={email} onChange={ (e) => setEmail(e.target.value) }/>
        </div>
        <div className="mb-5">
        ...
        <div className="mb-5">
          ...
          <textarea id="sintomas" className="border-2 w-full p-2 mt-2 placeholder-gray-400 rounded-md" placeholder="Sintomas" value={sintomas} onChange={ (e) => setSintomas(e.target.value) }/>
        </div>
        <button
          className="bg-indigo-600 w-full p-3 text-white uppercase font-bold hover:bg-indigo-700 cursor-pointer transition-colors"> {paciente.id ? "Editar Paciente" : "Agregar Paciente"}
        </button>
      </form>
    </div>
  )
